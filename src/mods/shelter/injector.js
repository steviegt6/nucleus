...
module.exports = () => {
          ...
          ? class extends BrowserWindow {
              constructor(opts) {
                if (opts.resizable && !injected) {
                  ...
                  fs.writeFileSync(
                    path.join(basePath, "preload.json"),
                    originalPreload
                  );
                  fs.writeFileSync(
                    path.join(basePath, "shelterPreload.js"),
                    shelterPreloadScript
                  );
                  opts.webPreferences.preload = path.join(
                    basePath,
                    "shelterPreload.js"
                  );
                  injected = true;
  ...
  (async () => {
    ...
    try {
      if (localBundle) {
        shelterBundle = readFileSync(
          resolve(join(localBundle, "shelter.js")),
          "utf8"
        );
        shelterBundle += `\n//# sourceMappingURL=file:////${resolve(
          join(localBundle, "shelter.js.map")
        )}`;
      } else {
      ...
      const options = {
        ...
        cancelId: 1,
        message:
          "Shelter failed to load from local dist. \nCheck console for more info.",
        detail: err.message,
      };
