...
exports.focusWindow = () => { var _a; return (_a = win === null || win === void 0 ? void 0 : win.focus) === null || _a === void 0 ? void 0 : _a.call(win); };
exports.pageReady = () => destroySplash() || process.nextTick(() => events.emit("APP_SHOULD_SHOW"));
const destroySplash = () => {
    var _a;
    (_a = win === null || win === void 0 ? void 0 : win.setSkipTaskbar) === null || _a === void 0 ? void 0 : _a.call(win, true);
    setTimeout(() => {
        if (!win)
            return;
        win.hide();
...
const sendState = (status, s = {}) => {
    try {
        win.webContents.send("state", Object.assign({ status }, s));
    }
    catch (_a) { }
};
...
class UIProgress {
    ...
    record(id, state, current, outOf) {
        this.total.add(id);
        if (current)
            this.progress.set(id, [current, outOf !== null && outOf !== void 0 ? outOf : 100]);
        if (state === "Complete")
            this.done.add(id);
        this.send();
    }
...
const initNew = (inst) => __awaiter(void 0, void 0, void 0, function* () {
    toSend = -1;
    const retryOptions = {
    ...
    while (true) {
        ...
        try {
            ...
            yield inst.updateToLatestWithOptions(retryOptions, ({ task, state, percent }) => {
                ...
                if (download != null)
                    simpleRecord(downloads, download);
                if (!downloads.send())
                    installs.send();
                if (install == null)
                    return;
            ...
            if (!installedAnything) {
                yield inst.startCurrentVersion();
                inst.collectGarbage();
                return launchMain();
            }
        }
...
const initOld = () => {
    ...
    on("checked", ({ failed, count }) => {
        ...
        if (failed)
            ...
            launchMain(); // Count is 0 / undefined
    });
