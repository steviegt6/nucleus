...
exports.do = (updater) => {
    ...
    try {
        ...
        for (const shortcut_path of [join(updater.getKnownFolder("desktop"), file), join(updater.getKnownFolder("programs"), Constants.APP_COMPANY, file)]) {
            if (!fs.existsSync(shortcut_path))
                continue; // Don't update already deleted paths
